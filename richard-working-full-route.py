...
def main():
    ...
    _ACCELERATION, _DECELERATION, _MIN_SPEED, _TURN_RATE = 0.07, 0.3, 0.3, 0.2
...
def _reset_motor_speeds():
    # The pwm counters theoretically could get close to 2.0 meaning that
    # speed adjustments take longer before gaving an effect
    # This sets them back to 1.0 if greater than 1.0
    # And is called when a pin first touches a line after going straight
    _pwm_counters[0] = min(_pwm_counters[0], 1.0)
    _pwm_counters[1] = min(_pwm_counters[1], 1.0)
